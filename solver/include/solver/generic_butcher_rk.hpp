...
namespace runge_kutta {
  ...
  struct diagonal_implicit_rk_butcher
  {
    ...
    using linear_algebra_t = typename std::conditional<
                              void_linear_algebra,
                              bool, // just to get a small type to store
                              LinearAlgebra_t
                            >::type;

    template <typename>
    diagonal_implicit_rk_butcher(double tol_=1e-10, std::size_t max_iter_=50)
    : butcher(), tol(tol_), max_iter(max_iter_)
    {}

    template <typename ... Args>
    diagonal_implicit_rk_butcher(double tol_=1e-10, std::size_t max_iter_=50, Args ... args )
    : butcher(), tol(tol_), max_iter(max_iter_), linalg(args...)
    {}

    template <typename ... Args> requires detail::has_newton_method<LinearAlgebra_t>
    ...
    stage ( Stage<I> , Problem_t & pb , value_t tn , state_t const& un , ArrayKi_t const& Ki , value_t dt )
    {
      ...
      using linear_algebra_t = typename std::conditional<
                        void_linear_algebra ,
                        ::ponio::linear_algebra::linear_algebra<matrix_t>,
                        LinearAlgebra_t
                      >::type;
      ...
      // $$
      //   dg : k \mapsto I - a_{ii}\Delta t \partial_k f( tn+c_i\Delta t, u^n + \Delta t \sum_{j=0}^{i-1} a_{ij}k_j + \Delta t a_{ii}k )
      // $$
      auto  g = [&]( state_t const& k ) -> state_t {
        return k - pb.f( tn + butcher.c[I]*dt , ::detail::tpl_inner_product<I>(butcher.A[I], Ki, un, dt) + dt*butcher.A[I][I]*k );
      };
      auto dg = [&]( state_t const& k) -> matrix_t {
        return identity - butcher.A[I][I]*dt*pb.df( tn + butcher.c[I]*dt , ::detail::tpl_inner_product<I>(butcher.A[I], Ki, un, dt) + dt*butcher.A[I][I]*k );
      };

      // call newton method
      if constexpr ( detail::has_newton_method<LinearAlgebra_t, decltype(g), decltype(dg)> || detail::has_newton_method<LinearAlgebra_t, decltype(g), decltype(dg), state_t> ) {
  ...
  make_dirk( Args ... args )
  {
    return diagonal_implicit_rk_butcher<Tableau, LinearAlgebra_t>(args...);
  }
  */
  /**
